...
AppleMenu *AppleMenu::__instance = NULL;
vector<Animation *> AppleMenu::animations = vector<Animation *>();

AppleMenu *AppleMenu::GetInstance()
{
	if (__instance == NULL)
		__instance = new AppleMenu();
	return __instance;
}
...
void AppleMenu::Update(DWORD dt)
{
	this->x = Viewport::GetInstance()->GetX() + 275;
	this->y = Viewport::GetInstance()->GetY() - 200;
}

void AppleMenu::Render()
{
